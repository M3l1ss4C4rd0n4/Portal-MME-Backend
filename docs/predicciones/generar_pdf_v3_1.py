...
modelos = [
    ...
    ("ENSEMBLE (combinación)", "Mezcla las predicciones de Prophet y ARIMA dándole más peso al que ha tenido mejor desempeño recientemente.", "Embalses (la combinación supera a cada modelo solo)"),
]
...
y = text_wrap(page, MARGIN, y, TEXT_W,
    "El Portal Energético del MME usa un modelo ENSEMBLE: combina las predicciones de Prophet y ARIMA estacional. No es un modelo 'preentrenado' como GPT-4. Se entrena desde cero con datos del Sistema Interconectado Nacional (SIN) de Colombia cada 3 días.",
    F, "helv", 10, C_BODY, 16)
...
y = text_wrap(page, MARGIN, y, TEXT_W,
    "El sistema se reentrena automáticamente cada 3 días (domingo, miércoles, sábado a las 2:00 AM) mediante Celery Beat, un programador de tareas. El pipeline completo toma entre 18 y 25 minutos.",
    F, "helv", 10, C_BODY, 16)
...
pasos = [
    ...
    ("PASO 2: PREPARACIÓN (1-2 segundos)", "Interpolación de fechas faltantes, winsorización de outliers (>3 desviaciones estándar), formateo para Prophet (columnas 'ds' y 'y') y ARIMA (índice temporal)."),
    ("PASO 3: ENTRENAMIENTO PARALELO (3-5 minutos)", "Prophet: optimización MAP (30-60 segundos). ARIMA: búsqueda stepwise de parámetros (2-3 minutos). Validación hold-out: últimos 30 días como prueba."),
    ...
    ("PASO 5: CALIBRACIÓN CONFORMAL (10-20 segundos)", "Entrena LightGBM auxiliar sobre features temporales. Calcula scores de calibración. Ajusta intervalos de confianza. Garantía: cobertura ≥ nivel de confianza."),
...
comparativa = [
    ...
    ("ARIMA est.", "4-32%", "2-3min", "Autocorrelación, IC estadísticos", "Requiere estacionariedad"),
...
y = draw_highlight_box(page, MARGIN, y + 5, TEXT_W, [
    ...
    "L_regularización: Evita que la red 'memorice' en lugar de 'aprender' (overfitting).",
    "λ: Pesos que balancean qué tan estricto es cada término. Se ajustan automáticamente."
...
y = draw_highlight_box(page, MARGIN, y + 5, TEXT_W, [
    ...
    "4) El percentil 90 de esos errores se suma/resta a la predicción del ensemble.",
...
for termino, definicion in glosario:
    draw_rect(page, MARGIN, y, TEXT_W, 38, C_NEAR_WHITE, C_BOX_BORDER)
    tw_write(page, [(MARGIN + 8, y + 10, termino + ":", FB, 9)], C_TITLE)
    y = text_wrap(page, MARGIN + 8, y + 22, TEXT_W - 16, definicion, F, "helv", 8, C_BODY, 12)
    y += 3
